...
// TestRetry проверяет что WithdrawWhenReady блокируется пока средств не хватит,
// и корректно завершается после пополнения.
func TestRetry(t *testing.T) {
	acc := NewAccount(50)
	done := make(chan struct{})

	go func() {
		WithdrawWhenReady(acc, 100)
		close(done)
	}()

	select {
	case <-done:
		t.Fatal("withdraw completed before deposit — should have blocked")
	case <-time.After(10 * time.Millisecond):
	}

	// Пополняем — горутина должна проснуться и завершиться.
	Atomically(func(tx *Txn) {
		tx.Write(acc, tx.Read(acc)+100)
	})

	select {
	case <-done:
	case <-time.After(time.Second):
		t.Fatal("withdraw did not complete after deposit")
	}

	if got := acc.Balance(); got != 50 {
		t.Errorf("want 50 (150-100), got %d", got)
	}
}
