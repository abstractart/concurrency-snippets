...
import (
	...
	"github.com/concurrency-examples/golang/internal/transfer_balance/pg_pessimistic"
)
...
func TestMain(m *testing.M) {
	...
	if err := pg_pessimistic.SetupSchema(ctx, db); err != nil {
...
func TestTransferConservation(t *testing.T) {
	...
	alice, _ := pg_pessimistic.CreateAccount(ctx, db, "alice", 1000)
	bob, _ := pg_pessimistic.CreateAccount(ctx, db, "bob", 1000)

	totalBefore, _ := pg_pessimistic.TotalBalance(ctx, db)
	...
	for range goroutines {
		go func() {
			defer wg.Done()
			if err := pg_pessimistic.Transfer(ctx, db, alice, bob, 10); err != nil {
				t.Errorf("transfer alice→bob: %v", err)
			}
		}()
		go func() {
			defer wg.Done()
			if err := pg_pessimistic.Transfer(ctx, db, bob, alice, 10); err != nil {
	...
	totalAfter, _ := pg_pessimistic.TotalBalance(ctx, db)
...
// Вероятность двух горутин попасть на одну пару невысока → блокировки почти не ждут.
func TestTransferManyAccounts(t *testing.T) {
	...
	for range goroutines {
		go func() {
			...
			if err := pg_pessimistic.Transfer(ctx, db, from, to, 1); err != nil {
