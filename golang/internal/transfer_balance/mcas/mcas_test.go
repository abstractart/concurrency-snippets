...
// BenchmarkLargeReadSet: MCAS не умеет атомарно читать без записи.
// Чтобы атомарно наблюдать N аккаунтов, нужно включить их все как write-entries
// (записать то же значение обратно). Здесь симулируем это: N readBalance() + 2-op commit.
// Это НИЖНЯЯ ГРАНИЦА стоимости — реальный N-op MCAS был бы ещё дороже (N CAS-цепочек).
func BenchmarkLargeReadSet(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		for pb.Next() {
			...
			for {
				// Наблюдаем все аккаунты (не атомарно — это ограничение MCAS)
				for _, a := range accounts {
					_ = a.readBalance()
				}
				t := &tx{}
				t.firstOp = newOperation(accounts[from], t, -1)
				t.secondOp = newOperation(accounts[to], t, +1)
				if uintptr(unsafe.Pointer(accounts[from])) > uintptr(unsafe.Pointer(accounts[to])) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			for {
				attempts.Add(1)
				t := &tx{}
				t.firstOp = newOperation(a1, t, -1)
				t.secondOp = newOperation(a2, t, +1)
				if uintptr(unsafe.Pointer(a1)) > uintptr(unsafe.Pointer(a2)) {
					t.firstOp, t.secondOp = t.secondOp, t.firstOp
				}
				if t.commit(); t.succeeded() {
					commits.Add(1)
